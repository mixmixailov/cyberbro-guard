...
def _cleanup_old_backups(backup_dir: Path, db_name: str, retention_count: int) -> None:
    ...
    if len(backup_files) <= retention_count:
        logger.debug("Found %d backup files, retention limit is %d - no cleanup needed", 
                    len(backup_files), retention_count)
        return
    ...
    logger.info("Cleaned up %d old backup files, kept %d recent backups", 
                len(files_to_remove), min(len(backup_files), retention_count))
