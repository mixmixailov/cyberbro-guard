...
from ..metrics import updates_total
...
class AdaptiveRateLimitMiddleware:
    ...
    async def check_rate_limit(self, scope: str, user_id: Optional[int] = None, 
        ...
        try:
            ...
            if not is_allowed:
                ...
                logger.debug(
                    "Rate limit check passed for scope %s (user_id=%s)",
                    scope, user_id
                )
    ...
    def create_rate_limit_handler(self, scope: str) -> Callable:
        ...
        """
        async def rate_limit_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[bool]:
