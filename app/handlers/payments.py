...
from app.services.payments import send_pro_invoice, handle_pre_checkout, handle_successful_payment, init_plans, refund_last
...
from app.db.payments import record_star_transaction, record_refunded_payment
...
async def plan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if sub and sub.get("until"):
        await msg.reply_text(t("plan.pro", lang=getattr(user, "language_code", None) or s.DEFAULT_LOCALE, until=sub.get("until")) + quota_line)
    else:
        await msg.reply_text(t("plan.free", lang=getattr(user, "language_code", None) or s.DEFAULT_LOCALE) + quota_line)
...
async def handle_star_transaction(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        logger.info("StarTransaction processed: user_id=%d, tx_id=%s, payment_id=%d", 
                   uid, star_tx.id, payment_id)
                   
    except Exception as exc:  # noqa: BLE001
        logger.error("StarTransaction processing error: user_id=%d, tx_id=%s, error=%s", 
                    uid, getattr(star_tx, 'id', 'unknown'), exc)
...
async def handle_refunded_payment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        logger.info("RefundedPayment processed: user_id=%d, charge_id=%s, payment_id=%d", 
                   uid, charge_id, payment_id)
        ...
        try:
            ...
            logger.warning("Failed to notify user about refund: user_id=%d, error=%s", 
                          uid, notify_exc)
                          
    except Exception as exc:  # noqa: BLE001
        logger.error("RefundedPayment processing error: user_id=%d, charge_id=%s, error=%s", 
                    uid, charge_id, exc)
...
def register(app: Application) -> None:
    ...
    if s.PAYMENTS_STARS_ENABLED:
        ...
        app.add_handler(MessageHandler(filters.MessageFilter(lambda msg: hasattr(msg, 'star_transaction')), handle_star_transaction))
        app.add_handler(MessageHandler(filters.MessageFilter(lambda msg: hasattr(msg, 'refunded_payment')), handle_refunded_payment))
