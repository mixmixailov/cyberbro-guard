...
from ..db.session import fetchall, execute
...
class AdaptiveRateLimitService:
    ...
    async def set_rate_limit_config(self, scope: str, rate_limit: float, 
                                   burst: float, cooldown: float) -> RateLimitConfig:
        ...
        logger.info(
            ...
            scope, rate_limit, burst, cooldown
        )
    ...
    async def get_token_bucket(self, scope: str) -> Optional[TokenBucket]:
        ...
        logger.debug("Created token bucket for scope %s: rate=%.1f, burst=%.1f", 
                    scope, config.rate_limit, config.burst)
