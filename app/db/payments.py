...
from .session import execute_immediate, fetchone
...
def record_star_transaction(
    ...
    if was_inserted:
        logger.info(
            "Recorded new StarTransaction: payment_id=%d, tx_id=%s, amount=%d",
            payment_id,
            transaction_id,
            amount,
        )
...
def record_refunded_payment(
    ...
    if was_inserted:
        logger.info(
            "Recorded new RefundedPayment: payment_id=%d, charge_id=%s, amount=%d",
            payment_id,
            charge_id,
            total_amount,
        )
