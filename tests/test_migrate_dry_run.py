...
from app.utils.migrate import _apply_sql_files, _get_migration_description, migrate
...
CREATE TABLE posts (
    ...
    def test_dry_run_no_database_changes(self):
        ...
        with (
            patch("app.utils.migrate.APP_DB_PATH", db_path),
    ...
    def test_dry_run_shows_migration_plan(self, caplog):
        ...
        with (
            ...
            # Run migration in dry-run mode
            migrate(str(self.migrations_dir), dry_run=True)
            ...
            assert any(
                "001_create_users - Create users table with basic fields" in msg
                for msg in log_messages
            )
            assert any(
                "002_add_timestamps - Add timestamp fields to users table" in msg
                for msg in log_messages
            )
            assert any(
                "003_create_posts - Create posts table with foreign key" in msg
                for msg in log_messages
            )

            # Should show dry-run completion
            assert any(
                "DRY RUN: Would apply 3 migrations (no changes made)" in msg for msg in log_messages
            )
...
class TestApplySqlFilesDryRun:
    ...
    def test_apply_sql_files_dry_run_no_execution(self, caplog):
        ...
        with patch("app.utils.migrate._get_conn") as mock_get_conn:
            ...
            log_messages = [record.message for record in caplog.records]
            assert any(
                "Would apply migration: 001_test - Test migration" in msg for msg in log_messages
            )
            assert any(
                "Would apply migration: 002_test - Another test" in msg for msg in log_messages
            )
...
class TestDryRunCLI:
    ...
    def test_cli_dry_run_flag(self, caplog):
        ...
        test_args = ["migrate", "--dir", str(self.migrations_dir), "--dry-run"]
        ...
        with (
            ...
            assert any(
                "Dry-run completed: would apply 1 migration(s)" in msg for msg in log_messages
            )
    ...
    def test_cli_without_dry_run_flag(self, caplog):
        ...
        # Mock command line arguments
        test_args = ["migrate", "--dir", str(self.migrations_dir)]
