...
from app.services.rate_limits import AdaptiveRateLimitService, RateLimitConfig, get_rate_limit_service
from app.middleware.adaptive_rate_limit import AdaptiveRateLimitMiddleware, get_rate_limit_middleware
...
class TestAdaptiveRateLimitMiddleware:
    ...
    async def test_check_rate_limit_allowed(self):
        ...
        with patch.object(self.middleware.rate_limit_service, 'get_token_bucket', 
        ...
        with patch('app.services.rate_limits.execute') as mock_execute:
            ...
            for i in range(5):
                task = asyncio.create_task(
                    self.service.set_rate_limit_config(f'scope{i}', float(i+1), float(i+10), float(i+20))
                )
                tasks.append(task)
